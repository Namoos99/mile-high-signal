...
| 3 | Warehouse (Postgres, star schema) | ⬜ Next |
...
make transform # clean with Spark, write processed/ Parquet + a readable CSV preview
make preview   # open the CSV preview — the easiest way to actually look at the data
```

`make test` runs the full suite with no network and no credentials.
...
number — see `docs/DECISIONS.md` AD-011 for why that matters). The alternative (advance first)
